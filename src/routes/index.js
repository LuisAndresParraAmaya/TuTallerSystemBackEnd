...
router.post('/ModifyProfile', async (req, res) => {
    ...
    WHERE user_rut = 195915741"`)
    if (response.length > 0) res.json({'Response': 'Login Success'})
    else res.json({'Response': 'Login Failed'})
})
